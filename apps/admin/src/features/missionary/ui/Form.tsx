...
import { useMissionaryForm } from '../model/use-form';
...
export function MissionaryForm({
  missionary,
  onSuccess,
  onCancel,
  isDialog = false,
}: Props) {
  const uiText = getFormText(missionary);

  const { form, imageUI, handler, status } = useMissionaryForm({
    missionary,
    onSuccess,
    ...
    <form onSubmit={handler.submit} className="space-y-4">
      ...
      <div className="space-y-2">
        <Label htmlFor="country">
          파송 국가 <span className="text-destructive">*</span>
        </Label>
        <Input
          id="country"
          className="h-12 text-base"
      ...
      <div className="space-y-2">
        <Label htmlFor="description">소개/활동 내용</Label>
        <Textarea
          id="description"
          className="min-h-24 text-base"
          placeholder="예: 현지 교회 개척 사역"
          disabled={status.isPending}
      ...
      <div className="flex justify-end gap-3 pt-4">
        <Button
          type="submit"
          size="lg"
          disabled={status.isPending || !isValid || !status.hasChanges}
        >
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>선교사 정보를 입력해주세요.</CardDescription>
      </CardHeader>
